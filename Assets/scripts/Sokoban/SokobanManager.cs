...
public class SokobanManager : MonoBehaviour
{
    ...
    public void setmap()
    {
        ...
        char[,,] char_map = sokoban_Parser.initializeMap();
        ...
        floors = new GameObject[countfloors];
    ...
    private void AgentMovements(bool isAgent)
    {
        if (Input.GetKeyDown(KeyCode.W))
        {
            foreach (GameObject tbox in boxes)
            {
                if (agent.GetComponent<Transform>().position + new Vector3(-1, 0, 0) == tbox.GetComponent<Transform>().position)
                {
                    ValidifyMove(tbox, new Vector3(-1, 0, 0));
                }
            }
            ValidifyMove(agent, new Vector3(-1, 0, 0));
        ...
        if (Input.GetKeyDown(KeyCode.A))
        {
            foreach (GameObject tbox in boxes)
            {
                if (agent.GetComponent<Transform>().position + new Vector3(0, 0, -1) == tbox.GetComponent<Transform>().position)
                    ValidifyMove(tbox, new Vector3(0, 0, -1));
            }
            ValidifyMove(agent, new Vector3(0, 0, -1));
        }
        ...
        if (Input.GetKeyDown(KeyCode.Q))
        {
            foreach (GameObject tbox in boxes)
            {
                if (agent.GetComponent<Transform>().position + new Vector3(0, 1, 0) == tbox.GetComponent<Transform>().position)
                    ValidifyMove(tbox, new Vector3(0, 1, 0));
            }
            ValidifyMove(agent, new Vector3(0, 1, 0));
        }
